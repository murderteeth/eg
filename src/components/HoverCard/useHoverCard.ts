...
const useHoverCardStore = create<HoverCardStore>((set) => ({
  isOpen: {},
  openHoverCard: (hoverCardId: string) =>
    set((state) => ({ isOpen: { ...state.isOpen, [hoverCardId]: true } })),
  closeHoverCard: (hoverCardId: string) =>
    set((state) => ({ isOpen: { ...state.isOpen, [hoverCardId]: false } })),
}))
...
export function useHoverCard(hoverCardId: string) {
  ...
  const closeHoverCard = useCallback(
    () => store.closeHoverCard(hoverCardId),
    [store, hoverCardId],
  )
