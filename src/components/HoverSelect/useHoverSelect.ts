...
const useHoverSelectStore = create<HoverSelectStore>((set) => ({
  ...
  multiValues: {},
  isOpen: {},
  setValue: (selectId: string, value: any) =>
    set((state) => ({ values: { ...state.values, [selectId]: value } })),
  ...
  openSelect: (selectId: string) =>
    set((state) => ({ isOpen: { ...state.isOpen, [selectId]: true } })),
  closeSelect: (selectId: string) =>
    set((state) => ({ isOpen: { ...state.isOpen, [selectId]: false } })),
}))
...
export function useHoverSelect<T = string>(
  ...
  const value = useMemo(() => store.values[selectId] ?? options?.defaultValue, [store.values, selectId, options?.defaultValue])
  ...
  const selectAllMultiValues = useCallback((values: T[]) => store.selectAllMultiValues(selectId, values), [store, selectId])
  
  const isSelected = useCallback((checkValue: T) => {
    if (options?.multiple) {
